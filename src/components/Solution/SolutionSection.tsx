    ...
    <section className="py-8 md:py-12 lg:py-16 bg-white dark:bg-dark-bg w-full">
      ...
      <div className="container mx-auto px-3 sm:px-4 lg:px-8">
        <div className="text-center mb-6 md:mb-8">
          ...
          <p className="font-montserrat text-lg text-gray-500 dark:text-dark-muted max-w-3xl mx-auto">
            {t.subtitle}
          </p>
        </div>
        {/* Comparativo */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-10">
          <div className="bg-[#fbe6f3] dark:bg-dark-surface rounded-xl shadow p-6 border-l-8 border-[#D90E8C]">
            <h3 className="font-orbitron font-bold text-lg text-[#D90E8C] mb-3">{t.tradicional.title}</h3>
            <ul className="space-y-2">
              {t.tradicional.items.map((item, idx) => (
                <li key={idx} className="flex items-start gap-2"><span className="text-[#D90E8C] font-bold">✗</span><span className="text-gray-700 dark:text-dark-text">{item}</span></li>
              ))}
            </ul>
          </div>
          <div className="bg-[#e9f6fc] dark:bg-dark-accent rounded-xl shadow p-6 border-l-8 border-[#139ED4]">
            <h3 className="font-orbitron font-bold text-lg text-[#139ED4] mb-3">{t.inxora.title}</h3>
            <ul className="space-y-2">
              {t.inxora.items.map((item, idx) => (
                <li key={idx} className="flex items-start gap-2"><span className="text-[#139ED4] font-bold">✓</span><span className="text-gray-700 dark:text-dark-text">{item}</span></li>
              ))}
            </ul>
          </div>
        </div>
